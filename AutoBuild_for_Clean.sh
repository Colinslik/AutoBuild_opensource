...
if [ "$2" = "i686" ]; then
  Cross_Compile=i686-asustor-linux-gnu
  Container_ID=asustor64
  File_Name=i686-asustor-linux-gnu.tar.gz
  Compile_Name="asustor/$File_Name"
elif [ "$2" = "x86_64" ]; then
  Cross_Compile=x86_64-asustor-linux-gnu
  Container_ID=asustor64
  File_Name=x86_64-asustor-linux-gnu.tar.gz
  Compile_Name="asustor/$File_Name"
else
  Cross_Compile=arm-marvell-linux-gnueabi
  Container_ID=asustor32
  File_Name=arm-marvell-linux-gnueabi_glibc2.22.tar.gz
  Compile_Name="asustor/$File_Name"
fi

Cross_Path=/home/colin/git_home/$Cross_Compile/bin/$Cross_Compile-

Download_Path="ftp://XXX.XXX.XXX.XXX/${Compile_Name} --ftp-user=XXX --ftp-password=XXX"
...
#=====get Base image ID=====
if [ "$Container_ID" = "asustor64" ]; then
  ORI_ID=$(docker images -q orbweb/ubuntu64)
else
...
make ; make install ; cp /home/colin/git_home/curl/lib/.libs/libcurl.a /home/colin/git_home/nattraversal-client/3rd-party/libcURL/unix/x86 ; \
expect -c "
   set timeout 1
...
expect -c "
   ...
   expect 100%
   sleep 1
   spawn scp /home/colin/git_home/nattraversal-client/build/linux/liborbwebM2M.so $Local_User@$Local_IP:$Dest
   expect yes/no { send yes\r ; exp_continue }
   expect password: { send $Local_Passwd\r }
   expect 100%
   sleep 1
